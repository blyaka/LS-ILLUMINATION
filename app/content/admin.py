...
class VideoAdmin(admin.ModelAdmin):
    list_display  = ('name','placement','position','status','created','preview','m3u8_link')
    list_filter   = ('placement','status')
    list_editable = ('placement','position')
    search_fields = ('name',)
    readonly_fields = ('status','m3u8_url','out_dir','meta','error','created','preview_tag')
    fields = ('name','description','source','placement','position','status','preview_tag','m3u8_url','out_dir','meta','error','created')
    ordering = ('placement','position','-created')

    def preview(self, obj):
        return format_html('<img src="{}" style="height:60px;border-radius:6px"/>',
                           obj.poster.url) if obj.poster else '-'

    def m3u8_link(self, obj):
        return format_html('<a href="{}" target="_blank">m3u8</a>',
                           obj.m3u8_url) if obj.m3u8_url else '-'

    def preview_tag(self, obj): return self.preview(obj)
    preview_tag.short_description = 'Постер'

    def get_urls(self):
        urls = super().get_urls()
        return [path('<uuid:pk>/rebuild/', self.admin_site.admin_view(self.rebuild),
                     name='content_video_rebuild')] + urls

    def rebuild(self, request, pk):
        try:
            v = Video.objects.get(pk=pk)
            v.status = Video.S.QUEUED; v.save(update_fields=['status'])
